    ...
    public void printStackTrace() {
        if (CollectionUtils.isEmpty(exceptionChains)) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                String message = String.format("[%s]:[%s] timestamp:'%s',exceptionClass:'%s',message:'%s',applicationName:'%s',path:'%s'", status, chain.getApplicationName(), format, chain.getExceptionClass(), chain.getMessage(), chain.getApplicationName(), chain.getPath());
                err.println(message);
                if (!isFinal) {
